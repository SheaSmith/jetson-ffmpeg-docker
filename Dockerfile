...
RUN  \
        ...
        mkdir -p ${DIR} && \
        cd ${DIR} && \
        curl -sLO https://download-mirror.savannah.gnu.org/releases/freetype/freetype-old/freetype-${FREETYPE_VERSION}.tar.gz && \
...
RUN ./configure \
        ...
        --extra-ldflags="-L${PREFIX}/lib" --enable-nvmpi --enable-libass && make -j4
